"""
environment/track.py – Stadium oval with chicane.

Road painted by thick-line stroking — zero polygon math, zero bleed.
All geometry verified: spine coords inside [130,920] x [100,720].
Optimized for readability and performance with type hints.
"""
import math
import random
...
def _arc(
    ...
    """Generate arc points using polar coordinates."""
    points = []
...
def _seg(
    ...
    """Generate line segment points with uniform spacing."""
...
def _build_spine() -> List[Tuple[float, float]]:
    """
    Build the race track centerline (spine) as a sequence of points.
    
    Circuit layout (clockwise, y↓):
    - Left straight (x=220, y: 590→210)
    - Top straight (y=120, x: 310→730) with chicane
    - Right straight (x=820, y: 210→590)
    - Bottom straight (y=680, x: 730→310)
    
    All corners are 90° arcs with radius=90.
    """
    points = []
    
    # 1. Left straight (going UP = y decreasing)
    points += _seg(LEFT_STRAIGHT_X, BOTTOM_LEFT_CENTER[1],
                   LEFT_STRAIGHT_X, TOP_LEFT_CENTER[1])
    
    # 2. Top-left arc: 180°→270°  (entry from south, exit going east)
    points += _arc(*TOP_LEFT_CENTER, CORNER_RADIUS, 180, 270)
    
    # 3. Top straight with chicane
    points += _seg(TOP_LEFT_CENTER[0], TOP_STRAIGHT_Y, 
                   CHICANE_START_X, TOP_STRAIGHT_Y)  # straight to chicane
    points += _seg(CHICANE_START_X, TOP_STRAIGHT_Y,
                   CHICANE_PEAK_X, TOP_STRAIGHT_Y + CHICANE_PEAK_OFFSET)  # chicane right
    points += _seg(CHICANE_PEAK_X, TOP_STRAIGHT_Y + CHICANE_PEAK_OFFSET,
                   CHICANE_END_X, TOP_STRAIGHT_Y)  # chicane left
    points += _seg(CHICANE_END_X, TOP_STRAIGHT_Y,
                   TOP_RIGHT_CENTER[0], TOP_STRAIGHT_Y)  # rest of top straight
    
    # 4. Top-right arc: 270°→360°
    points += _arc(*TOP_RIGHT_CENTER, CORNER_RADIUS, 270, 360)
    
    # 5. Right straight (going DOWN)
    points += _seg(RIGHT_STRAIGHT_X, TOP_RIGHT_CENTER[1],
                   RIGHT_STRAIGHT_X, BOTTOM_RIGHT_CENTER[1])
    
    # 6. Bottom-right arc: 0°→90°
    points += _arc(*BOTTOM_RIGHT_CENTER, CORNER_RADIUS, 0, 90)
    
    # 7. Bottom straight (going LEFT)
    points += _seg(BOTTOM_RIGHT_CENTER[0], BOTTOM_STRAIGHT_Y,
                   BOTTOM_LEFT_CENTER[0], BOTTOM_STRAIGHT_Y)
    
    # 8. Bottom-left arc: 90°→180°
    points += _arc(*BOTTOM_LEFT_CENTER, CORNER_RADIUS, 90, 180)
    
    # closed – last point = (LEFT_STRAIGHT_X, BOTTOM_LEFT_CENTER[1])
    # which is segment 1 start
    
    return points

# ─── stroke ──────────────────────────────────────────────────────────
def _stroke(surf, pts, color, width):
    r  = width // 2
    ip = [(int(x), int(y)) for x, y in pts]
    for i in range(len(ip)-1):
        pygame.draw.line(surf, color, ip[i], ip[i+1], width)
        pygame.draw.circle(surf, color, ip[i], r)
    pygame.draw.circle(surf, color, ip[-1], r)
...
class Track:
    ...
    def __init__(self):
        ...
        self.pothole_rects: List[pygame.Rect] = []  # For collision detection
        self.speedbreaker_rects: List[pygame.Rect] = []  # For collision detection
        self._spine: List[Tuple[float, float]] = _build_spine()
        self._build()
        
        # Build mask from GRASS: grass=wall(1), everything else=road(0)
        self.mask = pygame.mask.from_threshold(self.surface, GRASS, (20, 20, 20))
        
        # Paint start line after mask is finalized
    ...
    def _build(self) -> None:
        ...
        _stroke(surface, spine, GRAVEL, SW * 2)
        _stroke(surface, spine, ASPHALT, RW * 2)
